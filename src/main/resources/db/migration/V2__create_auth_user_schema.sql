...
CREATE UNIQUE INDEX uq_users_one_owner_per_group
    ON users(user_group_id) WHERE user_role = 'OWNER';
...
CREATE TABLE user_sessions (
                               ...
                               session_expires_at TIMESTAMPTZ NOT NULL DEFAULT (now() + interval '1 day'), -- fallback safety default; application should always set the real expiration
                               ...
                               CONSTRAINT chk_user_sessions_revocation_consistency
                                   CHECK (
                                       (session_revoked_at IS NULL AND session_revoke_reason IS NULL)
                                           OR (session_revoked_at IS NOT NULL AND session_revoke_reason IS NOT NULL)
                                       ),
...
CREATE INDEX idx_user_sessions_expires_at ON user_sessions(session_expires_at);
...
CREATE TABLE refresh_tokens (
                                ...
                                CONSTRAINT chk_refresh_tokens_revoked_after_created
                                    CHECK (refresh_token_revoked_at IS NULL OR refresh_token_revoked_at >= refresh_token_created_at),
                                ...
                                CONSTRAINT chk_refresh_tokens_reuse_reason_consistent
                                    CHECK (
                                        refresh_token_reuse_detected_at IS NULL
                                            OR (refresh_token_revoked_at IS NOT NULL
                                     ...
                                     CONSTRAINT chk_auth_login_attempts_failure_reason
                                         CHECK (
                                             (login_successful = true AND failure_reason IS NULL)
                                                 OR
                                             (login_successful = false AND failure_reason IN (
                                                                                              'INVALID_CREDENTIALS',
                                                                                              'USER_DISABLED',
                                                                                              'RATE_LIMITED'
                                                 ))
